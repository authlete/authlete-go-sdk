...
```go
authlete.WithServerURL("https://api.example.com")
```
...
```go
authlete.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
authlete.WithClient(httpClient)
```
...
```go
authlete.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
authlete.WithTimeout(30 * time.Second)
```
