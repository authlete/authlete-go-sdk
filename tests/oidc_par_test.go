...
func TestOIDCParFlow(t *testing.T) {
	...
	t.Run("OidcParFlow", func(t *testing.T) {
		nonce := randomNonce(t)
		encodedRedirect := url.QueryEscape(redirectURI)

		// Step 1: PAR with openid scope and nonce.
		...
		require.NotNil(t, tok.IDToken, "id_token must be issued for an openid PAR flow")
